...
-- Set all existing empresa ADMIN cargo permissions to true
UPDATE public.empresa_cargo_permissoes
SET permitido = true
WHERE empresa_cargo_id IN (
  SELECT id FROM public.empresa_cargos_config WHERE nome = 'ADMIN'
);

-- Update trigger to default permissions to true for ADMIN roles
